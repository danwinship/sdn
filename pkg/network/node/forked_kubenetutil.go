...
func chooseIPFromHostInterfaces(family AddressFamily, nw networkInterfacer) (net.IP, error) {
...
func ChooseHostInterface(family AddressFamily) (net.IP, error) {
	...
	if _, err := os.Stat(ipv4RouteFile); os.IsNotExist(err) {
		return chooseIPFromHostInterfaces(family, nw)
	}
	routes, err := getAllDefaultRoutes()
	if err != nil {
		return nil, err
	}
	return chooseHostInterfaceFromRoute(routes, family, nw)
}
...
func chooseHostInterfaceFromRoute(routes []Route, family AddressFamily, nw networkInterfacer) (net.IP, error) {
	{
